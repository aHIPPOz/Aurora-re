package com.google.android.gms.internal.ads;

import android.content.Context;
import com.google.android.gms.ads.RequestConfiguration;
import com.google.common.util.concurrent.ListenableFuture;
import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.security.MessageDigest;
import java.security.NoSuchAlgorithmException;
import java.util.concurrent.Callable;
import java.util.concurrent.Executor;
/* compiled from: com.google.android.gms:play-services-ads@@24.5.0 */
/* loaded from: classes2.dex */
public final class zzaup {
    private final ListenableFuture zza;
    private final zzavx zzb;
    private final zzasp zzc;

    public zzaup(final Context context, Executor executor, zzasp zzaspVar, zzavx zzavxVar) {
        this.zzb = zzavxVar;
        this.zzc = zzaspVar;
        this.zza = zzgdn.zzj(new Callable() { // from class: com.google.android.gms.internal.ads.zzaun
            @Override // java.util.concurrent.Callable
            public final Object call() {
                String zzd;
                zzd = zzaup.this.zzd(context);
                return zzd;
            }
        }, executor);
    }

    private static String zzc(Context context) {
        File file = new File(context.getPackageResourcePath());
        if (!file.exists() || !file.canRead()) {
            return RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED;
        }
        try {
            FileInputStream fileInputStream = new FileInputStream(file);
            byte[] bArr = new byte[16384];
            MessageDigest messageDigest = MessageDigest.getInstance("SHA256");
            for (int read = fileInputStream.read(bArr); read != -1; read = fileInputStream.read(bArr)) {
                messageDigest.update(bArr, 0, read);
            }
            zzgbd zzf = zzgbd.zzi().zzf();
            byte[] digest = messageDigest.digest();
            String zzj = zzf.zzj(digest, 0, digest.length);
            fileInputStream.close();
            return zzj;
        } catch (IOException | UnsupportedOperationException | NoSuchAlgorithmException unused) {
            return RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED;
        }
    }

    /* JADX WARN: Removed duplicated region for block: B:25:0x0055 A[Catch: InterruptedException | ExecutionException -> 0x00cb, TRY_ENTER, TRY_LEAVE, TryCatch #3 {InterruptedException | ExecutionException -> 0x00cb, blocks: (B:22:0x004f, B:25:0x0055, B:30:0x00bd, B:26:0x005a, B:28:0x008d, B:29:0x00a7), top: B:47:0x004f }] */
    /* JADX WARN: Removed duplicated region for block: B:26:0x005a A[Catch: all -> 0x00b9, TRY_ENTER, TryCatch #3 {InterruptedException | ExecutionException -> 0x00cb, blocks: (B:22:0x004f, B:25:0x0055, B:30:0x00bd, B:26:0x005a, B:28:0x008d, B:29:0x00a7), top: B:47:0x004f }] */
    /* JADX WARN: Removed duplicated region for block: B:33:0x00ca  */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
    public final java.lang.String zzd(android.content.Context r12) {
        /*
            r11 = this;
            com.google.android.gms.internal.ads.zzavx r0 = r11.zzb
            r1 = 31
            java.lang.String r2 = "E"
            if (r0 == 0) goto L42
            com.google.android.gms.internal.ads.zzasp r3 = r11.zzc
            boolean r4 = r3.zzf()
            if (r4 != 0) goto L42
            com.google.common.util.concurrent.ListenableFuture r4 = r0.zza()
            if (r4 == 0) goto L42
            int r4 = android.os.Build.VERSION.SDK_INT
            if (r4 < r1) goto L24
            com.google.common.util.concurrent.ListenableFuture r4 = r0.zza()
            boolean r4 = r4.isDone()
            if (r4 == 0) goto L42
        L24:
            com.google.common.util.concurrent.ListenableFuture r0 = r0.zza()     // Catch: java.lang.Throwable -> L42
            int r3 = r3.zza()     // Catch: java.lang.Throwable -> L42
            long r3 = (long) r3     // Catch: java.lang.Throwable -> L42
            java.util.concurrent.TimeUnit r5 = java.util.concurrent.TimeUnit.MILLISECONDS     // Catch: java.lang.Throwable -> L42
            java.lang.Object r0 = r0.get(r3, r5)     // Catch: java.lang.Throwable -> L42
            com.google.android.gms.internal.ads.zzatq r0 = (com.google.android.gms.internal.ads.zzatq) r0     // Catch: java.lang.Throwable -> L42
            if (r0 == 0) goto L42
            boolean r3 = r0.zzaf()     // Catch: java.lang.Throwable -> L42
            if (r3 == 0) goto L42
            java.lang.String r0 = r0.zzg()     // Catch: java.lang.Throwable -> L42
            goto L43
        L42:
            r0 = r2
        L43:
            boolean r3 = r0.equals(r2)
            if (r3 == 0) goto Lcb
            int r3 = android.os.Build.VERSION.SDK_INT
            if (r3 >= r1) goto L4f
            goto Lcb
        L4f:
            int r3 = android.os.Build.VERSION.SDK_INT     // Catch: java.lang.Throwable -> Lcb
            java.lang.String r4 = ""
            if (r3 >= r1) goto L5a
            com.google.common.util.concurrent.ListenableFuture r1 = com.google.android.gms.internal.ads.zzgdn.zzh(r4)     // Catch: java.lang.Throwable -> Lcb
            goto Lbd
        L5a:
            java.lang.String r6 = r12.getPackageName()     // Catch: java.lang.Throwable -> Lb9
            java.lang.String r1 = "X.509"
            java.security.cert.CertificateFactory r1 = java.security.cert.CertificateFactory.getInstance(r1)     // Catch: java.lang.Throwable -> Lb9
            com.google.android.gms.internal.ads.zzgbd r3 = com.google.android.gms.internal.ads.zzgbd.zzi()     // Catch: java.lang.Throwable -> Lb9
            com.google.android.gms.internal.ads.zzgbd r3 = r3.zzf()     // Catch: java.lang.Throwable -> Lb9
            java.lang.String r5 = ""
            byte[] r3 = r3.zzk(r5)     // Catch: java.lang.Throwable -> Lb9
            java.util.ArrayList r9 = new java.util.ArrayList     // Catch: java.lang.Throwable -> Lb9
            r9.<init>()     // Catch: java.lang.Throwable -> Lb9
            java.io.ByteArrayInputStream r5 = new java.io.ByteArrayInputStream     // Catch: java.lang.Throwable -> Lb9
            r5.<init>(r3)     // Catch: java.lang.Throwable -> Lb9
            java.security.cert.Certificate r3 = r1.generateCertificate(r5)     // Catch: java.lang.Throwable -> Lb9
            r9.add(r3)     // Catch: java.lang.Throwable -> Lb9
            java.lang.String r3 = android.os.Build.TYPE     // Catch: java.lang.Throwable -> Lb9
            java.lang.String r5 = "user"
            boolean r3 = r3.equals(r5)     // Catch: java.lang.Throwable -> Lb9
            if (r3 != 0) goto La7
            com.google.android.gms.internal.ads.zzgbd r3 = com.google.android.gms.internal.ads.zzgbd.zzi()     // Catch: java.lang.Throwable -> Lb9
            com.google.android.gms.internal.ads.zzgbd r3 = r3.zzf()     // Catch: java.lang.Throwable -> Lb9
            java.lang.String r5 = ""
            byte[] r3 = r3.zzk(r5)     // Catch: java.lang.Throwable -> Lb9
            java.io.ByteArrayInputStream r5 = new java.io.ByteArrayInputStream     // Catch: java.lang.Throwable -> Lb9
            r5.<init>(r3)     // Catch: java.lang.Throwable -> Lb9
            java.security.cert.Certificate r1 = r1.generateCertificate(r5)     // Catch: java.lang.Throwable -> Lb9
            r9.add(r1)     // Catch: java.lang.Throwable -> Lb9
        La7:
            com.google.android.gms.internal.ads.zzauo r10 = new com.google.android.gms.internal.ads.zzauo     // Catch: java.lang.Throwable -> Lb9
            r10.<init>()     // Catch: java.lang.Throwable -> Lb9
            android.content.pm.PackageManager r5 = r12.getPackageManager()     // Catch: java.lang.Throwable -> Lb9
            r7 = 0
            r8 = 8
            r5.requestChecksums(r6, r7, r8, r9, r10)     // Catch: java.lang.Throwable -> Lb9
            com.google.android.gms.internal.ads.zzgeh r1 = r10.zza     // Catch: java.lang.Throwable -> Lb9
            goto Lbd
        Lb9:
            com.google.common.util.concurrent.ListenableFuture r1 = com.google.android.gms.internal.ads.zzgdn.zzh(r4)     // Catch: java.lang.Throwable -> Lcb
        Lbd:
            java.lang.Object r1 = r1.get()     // Catch: java.lang.Throwable -> Lcb
            java.lang.String r1 = (java.lang.String) r1     // Catch: java.lang.Throwable -> Lcb
            boolean r3 = com.google.android.gms.internal.ads.zzfwg.zzd(r1)     // Catch: java.lang.Throwable -> Lcb
            r4 = 1
            if (r4 == r3) goto Lcb
            r0 = r1
        Lcb:
            boolean r1 = r0.equals(r2)
            if (r1 == 0) goto Ldd
            com.google.android.gms.internal.ads.zzasp r1 = r11.zzc
            boolean r1 = r1.zzd()
            if (r1 != 0) goto Ldd
            java.lang.String r0 = zzc(r12)     // Catch: java.lang.ClassCastException -> Ldd
        Ldd:
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzaup.zzd(android.content.Context):java.lang.String");
    }

    public final ListenableFuture zza() {
        return this.zza;
    }
}
