...
public class LinkedListMultimap<K, V> extends AbstractMultimap<K, V> implements ListMultimap<K, V>, Serializable {
    ...
    public class NodeIterator implements ListIterator<Map.Entry<K, V>> {
        ...
        NodeIterator(int i) {
            ...
            this.expectedModCount = r3.modCount;
            int size = r3.size();
            Preconditions.checkPositionIndex(i, size);
            if (i >= size / 2) {
                this.previous = r3.tail;
                this.nextIndex = size;
                while (true) {
                    int i2 = i + 1;
                    if (i >= size) {
    ...
    private class DistinctKeyIterator implements Iterator<K> {
        ...
        private DistinctKeyIterator() {
            LinkedListMultimap.this = r2;
            this.seenKeys = Sets.newHashSetWithExpectedSize(r2.keySet().size());
            this.next = r2.head;
    ...
    public class ValueForKeyIterator implements ListIterator<V> {
        ...
        ValueForKeyIterator(@ParametricNullness K k) {
            ...
            this.key = k;
            KeyList keyList = (KeyList) r1.keyToKeyList.get(k);
            this.next = keyList == null ? null : keyList.head;
        }

        public ValueForKeyIterator(@ParametricNullness K k, int i) {
            LinkedListMultimap.this = r4;
            KeyList keyList = (KeyList) r4.keyToKeyList.get(k);
