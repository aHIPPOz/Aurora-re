    ...
    */
    ...
    public void processTouchEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        if (action == 0) {
            ...
            char c4 = c2 == 1 ? 1 : 0;
            char c5 = c2 == 1 ? 1 : 0;
            float f8 = f7 * fArr4[c4];
            float f9 = this.mTouchDirectionY;
            char c6 = c3 == 1 ? 1 : 0;
            char c7 = c3 == 1 ? 1 : 0;
            if (Math.abs((f8 + (f9 * fArr4[c6])) * this.mDragScale) < 0.01d) {
                float[] fArr5 = this.mAnchorDpDt;
                fArr5[c2] = 0.01f;
                fArr5[c3] = 0.01f;
            }
            if (this.mTouchDirectionX != 0.0f) {
                f2 = rawX / this.mAnchorDpDt[c2];
            } else {
                f2 = rawY / this.mAnchorDpDt[c3];
            }
            ...
            if (max != progress3) {
                ...
                if (i4 == 0 || progress3 == 1.0f) {
                    ...
                    if (i4 == 0) {
                        boolean z2 = c3 == 1 ? 1 : 0;
                        Object[] objArr = c3 == 1 ? 1 : 0;
                        z = z2;
                    } else {
                        boolean z3 = c2 == 1 ? 1 : 0;
                        Object[] objArr2 = c2 == 1 ? 1 : 0;
                        z = z3;
                    }
                ...
                velocityTracker.computeCurrentVelocity(1000);
                this.mMotionLayout.mLastVelocity = this.mTouchDirectionX != 0.0f ? velocityTracker.getXVelocity() / this.mAnchorDpDt[c2] : velocityTracker.getYVelocity() / this.mAnchorDpDt[c3];
            } else {
    ...
    public void scrollUp(float dx, float dy) {
        ...
        float[] fArr = this.mAnchorDpDt;
        float f2 = 0.0f;
        float f3 = f != 0.0f ? (dx * f) / fArr[0] : (dy * this.mTouchDirectionY) / fArr[1];
        if (!Float.isNaN(f3)) {
            progress += f3 / 3.0f;
        }
        if (progress != 0.0f) {
            ...
            motionLayout.touchAnimateTo(i, f2, f3);
        }
    }
