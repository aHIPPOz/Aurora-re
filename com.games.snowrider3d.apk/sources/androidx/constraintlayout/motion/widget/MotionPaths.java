...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f18 = 0.0f;
        float f19 = Float.isNaN(c.mAltPercentY) ? 0.0f : c.mAltPercentY;
        ...
        this.mMode = 0;
        this.x = (int) (((startTimePoint.x + (f17 * f11)) + (f18 * f12)) - f14);
        this.y = (int) (((startTimePoint.y + (f11 * f19)) + (f12 * f10)) - f16);
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f12 = f3;
        float f13 = (endTimePoint.x + (endTimePoint.width / 2.0f)) - ((f9 / 2.0f) + f8);
        float f14 = (endTimePoint.y + (endTimePoint.height / 2.0f)) - ((f11 / 2.0f) + f10);
        float f15 = f13 * f12;
        float f16 = (f6 * f4) / 2.0f;
        this.x = (int) ((f8 + f15) - f16);
        float f17 = f14 * f12;
        float f18 = (f7 * f5) / 2.0f;
        this.y = (int) ((f10 + f17) - f18);
        this.width = (int) (f9 + f);
        ...
        this.x = f20;
        this.x = f20 + ((-f14) * f19);
        this.y = ((int) ((startTimePoint.y + f17) - f18)) + (f13 * f19);
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, double[] vdata, float[] velocity) {
        ...
        if (motionController != null) {
            ...
            f12 = (float) (f16 + (Math.sin(d2) * d3) + (Math.cos(d2) * d4));
        } else {
            f = 2.0f;
        }
    ...
    public void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        if (motionController != null) {
            ...
            float cos = (float) ((f17 - (d6 * Math.cos(d4))) + (Math.sin(d4) * d3 * d7));
            if (slope.length >= 2) {
                slope[0] = sin;
                slope[1] = cos;
            }
            if (!Float.isNaN(f13)) {
                view.setRotation((float) (f13 + Math.toDegrees(Math.atan2(cos, sin))));
            }
