...
public class Debug {
    ...
    public static String getName(Context context, int[] id) {
        ...
        try {
            ...
            int i = 0;
            while (i < id.length) {
                String str3 = str2 + (i == 0 ? RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED : " ");
                try {
    ...
    public static String getState(MotionLayout layout, int stateId, int len) {
        ...
        return (resourceEntryName.length() <= len || (length = resourceEntryName.replaceAll("[^_]", RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED).length()) <= 0) ? resourceEntryName : resourceEntryName.replaceAll(CharBuffer.allocate((resourceEntryName.length() - len) / length).toString().replace((char) 0, '.') + "_", "_");
    }
